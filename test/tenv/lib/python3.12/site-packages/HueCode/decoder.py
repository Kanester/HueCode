...
class Decrypt:
  ...
  def getInfo(self):
    ...
    downscaled = image.resize((image.width // self.SCALE, image.height // self.SCALE), resample=Image.NEAREST)
  ...
  def reverse(self, color, channel):
    ...
    return round((((color / 255) * (127 * shift_value + self.FACTOR)) - self.FACTOR) / shift_value)
